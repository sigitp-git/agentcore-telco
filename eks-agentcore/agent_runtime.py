#!/usr/bin/env python3
"""
Amazon Bedrock AgentCore Runtime version of the EKS Agent.
...
def create_runtime_agent():
    ...
    try:
        ...
        agent = Agent(
            ...
            tools=tools,
            hooks=hooks,
            system_prompt="""You are an EKS Agent, an expert AWS Kubernetes and container orchestration assistant. You help with:

- Amazon EKS cluster management and configuration
- Kubernetes workload deployment and scaling
- Container security and best practices
- EKS networking and service mesh (Istio, App Mesh)
- Kubernetes troubleshooting and debugging
- EKS add-ons and integrations (ALB Controller, EBS CSI, etc.)
...
Focus on EKS best practices, security, and cost optimization for Kubernetes workloads."""
        )
...
1. Use web search ONLY when:
   ...
   - User asks about very recent EKS/Kubernetes announcements or updates  
   - User needs current pricing or availability information
2. Use MCP tools when appropriate for advanced functionality
3. For general EKS questions, greetings, or basic Kubernetes topics, use your existing knowledge without searching
...
if __name__ == "__main__":
    ...
    logger.info("Starting EKS Agent Runtime Server...")
