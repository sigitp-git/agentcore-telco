#!/usr/bin/env python3
"""
Streamlit Frontend for AWS Outposts Agent
A user-friendly web interface for interacting with the deployed Outposts Agent.
"""
...
class StreamlitAgentInterface:
    """Streamlit interface for the Outposts Agent."""
    ...
    def get_agent_runtime_arn(self):
        """Get the agent runtime ARN for Outposts Agent from environment variable."""
        # Get Outposts Agent runtime ARN from environment variable
        arn = os.getenv("OUTPOSTS_AGENT_RUNTIME_ARN", "arn:aws:bedrock-agentcore:us-east-1:ACCOUNT_ID:runtime/outposts_agent-RUNTIME_ID")
        return arn
    
    def get_agent_gateway_id(self):
        """Get the agent gateway ID for Outposts Agent from SSM parameter."""
        try:
            gateway_id = get_ssm_parameter("/app/outpostsagent/agentcore/gateway_id")
            if gateway_id:
                return gateway_id
        except Exception:
            pass
        ...
        # Get Outposts Agent gateway ID from environment variable
        gateway_id = os.getenv("OUTPOSTS_AGENT_GATEWAY_ID", "outposts-agent-agentcore-gw-GATEWAY_ID")
        return gateway_id
    ...
    def invoke_agent(self, prompt, session_id=None):
        ...
        try:
            ...
            with st.spinner('🤖 Outposts Agent is thinking...'):
                # Invoke the agent
...
def main():
    ...
    # Header
    st.markdown('<h1 class="main-header">🚀 AWS Outposts Agent</h1>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        example_prompts = [
            "How do I plan and deploy AWS Outposts racks?",
            "What services are available on AWS Outposts?",
            "Help me troubleshoot Outposts connectivity issues",
            "How do I manage local storage on Outposts?",
            "What are best practices for hybrid cloud architecture?",
            "How do I ensure data residency with Outposts?"
        ]
        ...
        st.markdown("""
        - **Outposts Deployment & Management**
        - **Hybrid Cloud Architecture**
        - **Edge Computing Workloads**
        - **Outposts Networking & Connectivity**
        - **Local Storage Management**
        - **Service Compatibility & Limitations**
    ...
    # Main chat interface
    st.header("💬 Chat with Outposts Agent")
    ...
    for message in st.session_state.messages:
        if message["role"] == "user":
            ...
            <div class="chat-message agent-message">
                <strong>🤖 Outposts Agent:</strong><br>
                {message["content"]}
            </div>
            ''', unsafe_allow_html=True)
    ...
    <div style="text-align: center; color: #666; margin-top: 2rem;">
        <p>🚀 <strong>AWS Outposts Agent</strong> | Powered by Amazon Bedrock & AgentCore Runtime</p>
