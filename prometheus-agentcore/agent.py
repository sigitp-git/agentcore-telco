...
class AgentConfig:
    ...
    # MCP Configuration, AgentCore Gateway MCP tools
    ENABLE_MCP_CONFIG = False  # Toggle to enable/disable AgentCore Gateway MCP config loading
    MCP_CONFIG_PATH = ''
    ...
    SSM_MEMORY_ID_PATH = "/app/prometheusagent/agentcore/memory_id"
    MEMORY_EXPIRY_DAYS = 90
    CONTEXT_RETRIEVAL_TOP_K = 3
    DEVOPS_USER_ID = "prometheus_001"
...
def validate_gateway_configuration():
    ...
    required_params = {
        "/app/prometheusagent/agentcore/machine_client_id": "Machine Client ID",
        "/app/prometheusagent/agentcore/cognito_discovery_url": "Cognito Discovery URL",
        "/app/prometheusagent/agentcore/gateway_iam_role": "Gateway IAM Role"
    }
...
def use_manual_gateway(region=None):
    ...
    try:
        ...
        put_ssm_parameter("/app/prometheusagent/agentcore/gateway_id", manual_gateway_id)
...
#     
#     gateway_client = boto3.client("bedrock-agentcore-control", region_name=REGION)
#     gateway_name = "prometheusagent-agentcore-gw"
#     
#     # Get configuration
...
#             name=gateway_name,
#             roleArn=get_ssm_parameter("/app/prometheusagent/agentcore/gateway_iam_role"),
#             protocolType="MCP",
#             authorizerType="CUSTOM_JWT",
#             authorizerConfiguration=auth_config,
#             description="Prometheus Agent AgentCore Gateway",
#         )
#         
...
#         }
#         put_ssm_parameter("/app/prometheusagent/agentcore/gateway_id", gateway_id)
...
def try_existing_gateway(gateway_client):
    ...
    existing_gateway_id = get_ssm_parameter("/app/prometheusagent/agentcore/gateway_id")
...
def get_token(client_id: str, client_secret: str, scope_string: str = None, url: str = None) -> dict:
    try:
        ...
        if scope_string is None:
            scope_string = get_ssm_parameter("/app/prometheusagent/agentcore/cognito_auth_scope") or "openid"
        if url is None:
            url = get_ssm_parameter("/app/prometheusagent/agentcore/cognito_token_url")
            if not url or "REDACTED_COGNITO_ID" in url:
                # Skip token request if no valid URL is configured
                return {"error": "Cognito token URL not configured. Please set /app/prometheusagent/agentcore/cognito_token_url SSM parameter."}
...
class PrometheusAgentMemoryHooks(HookProvider):
    ...
    def retrieve_prometheus_context(self, event: MessageAddedEvent):
        """Retrieve Prometheus context before processing query"""
        ...
        if (
            ...
            try:
                ...
                for context_type, namespace in self.namespaces.items():
                    # *** AGENTCORE MEMORY USAGE *** - Retrieve Prometheus context from each namespace
                ...
                # Inject Prometheus context into the query
                if all_context:
                    context_text = "\n".join(all_context)
                    original_text = messages[-1]["content"][0]["text"]
                    messages[-1]["content"][0][
                        "text"
                    ] = f"Prometheus Context:\n{context_text}\n\n{original_text}"
                    logger.info(f"Retrieved {len(all_context)} Prometheus context items")

            except Exception as e:
                logger.error(f"Failed to retrieve Prometheus context: {e}")

    def save_prometheus_interaction(self, event: AfterInvocationEvent):
        """Save Prometheus Agent interaction after agent response"""
        try:
            messages = event.agent.messages
...
def cleanup_all_resources():
    ...
    # Cleanup AWS MCP Manager
    if aws_mcp_manager:
        try:
            aws_mcp_manager.cleanup()
        except Exception as e:
            print(f"⚠️  AWS MCP cleanup error: {e}")
...
def setup_gateway_and_mcp():
    ...
    # Initialize AWS MCP Manager after MCP client is ready
    if AgentConfig.ENABLE_AWS_MCP:
        try:
            print("🔧 Initializing AWS MCP integration...")
