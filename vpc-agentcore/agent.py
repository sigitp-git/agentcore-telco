...
class AgentConfig:
    """Configuration settings for the VPC Agent."""
    ...
    # Memory Settings
    MEMORY_NAME = "VPCAgentMemory"
    SSM_MEMORY_ID_PATH = "/app/vpcagent/agentcore/memory_id"
    MEMORY_EXPIRY_DAYS = 90
    CONTEXT_RETRIEVAL_TOP_K = 3
    DEVOPS_USER_ID = "vpc_001"
    
    # Search Settings
    SEARCH_REGION = "us-en"
    
    # System Prompt
    VPC_SYSTEM_PROMPT = """You are an AWS VPC networking specialist agent. You help with Virtual Private Cloud (VPC) architecture, networking, connectivity, security groups, routing, and troubleshooting.
...
def validate_gateway_configuration():
    ...
    required_params = {
        "/app/vpcagent/agentcore/machine_client_id": "Machine Client ID",
        "/app/vpcagent/agentcore/cognito_discovery_url": "Cognito Discovery URL",
        "/app/vpcagent/agentcore/gateway_iam_role": "Gateway IAM Role"
    }
...
def use_manual_gateway(region=None):
    ...
    manual_gateway_id = os.getenv("VPC_AGENT_GATEWAY_ID", "vpc-agent-agentcore-gw-cjhql5d31l")
    ...
    try:
        ...
        put_ssm_parameter("/app/vpcagent/agentcore/gateway_id", manual_gateway_id)
...
#     
#     gateway_client = boto3.client("bedrock-agentcore-control", region_name=REGION)
#     gateway_name = "vpcagent-agentcore-gw"
#     
#     # Get configuration
...
#             name=gateway_name,
#             roleArn=get_ssm_parameter("/app/vpcagent/agentcore/gateway_iam_role"),
#             protocolType="MCP",
#             authorizerType="CUSTOM_JWT",
#             authorizerConfiguration=auth_config,
#             description="VPC Agent AgentCore Gateway",
#         )
#         
...
#         }
#         put_ssm_parameter("/app/vpcagent/agentcore/gateway_id", gateway_id)
...
def try_existing_gateway(gateway_client):
    ...
    existing_gateway_id = get_ssm_parameter("/app/vpcagent/agentcore/gateway_id")
...
def get_token(client_id: str, client_secret: str, scope_string: str = None, url: str = None) -> dict:
    try:
        ...
        if scope_string is None:
            scope_string = get_ssm_parameter("/app/vpcagent/agentcore/cognito_auth_scope") or "openid"
        if url is None:
            url = get_ssm_parameter("/app/vpcagent/agentcore/cognito_token_url")
            if not url or "REDACTED_COGNITO_ID" in url:
                # Skip token request if no valid URL is configured
                return {"error": "Cognito token URL not configured. Please set /app/vpcagent/agentcore/cognito_token_url SSM parameter."}
...
logging.getLogger("awslabs.vpc_mcp_server").setLevel(logging.WARNING)  # Reduce VPC MCP server logging
        ...
        return [
            {
                StrategyType.USER_PREFERENCE.value: {
                    "name": "VPCPreferences",
                    "description": "Captures VPC preferences and behavior",
                    "namespaces": ["agent/vpc/{actorId}/preferences"],
                }
            },
            {
                StrategyType.SEMANTIC.value: {
                    "name": "VPCAgentSemantic",
                    "description": "Stores facts from conversations",
                    "namespaces": ["agent/vpc/{actorId}/semantic"],
                }
            },
        ]
...
class VPCAgentMemoryHooks(HookProvider):
    """Memory hooks for VPC Agent"""
    ...
    def retrieve_vpc_context(self, event: MessageAddedEvent):
        """Retrieve VPC context before processing query"""
        ...
        if (
            ...
            try:
                ...
                for context_type, namespace in self.namespaces.items():
                    # *** AGENTCORE MEMORY USAGE *** - Retrieve VPC context from each namespace
                ...
                # Inject VPC context into the query
                if all_context:
                    context_text = "\n".join(all_context)
                    ...
                    logger.info(f"Retrieved {len(all_context)} VPC context items")

            except Exception as e:
                logger.error(f"Failed to retrieve VPC context: {e}")

    def save_vpc_interaction(self, event: AfterInvocationEvent):
        """Save VPC Agent interaction after agent response"""
        try:
            messages = event.agent.messages
    ...
    def register_hooks(self, registry: HookRegistry) -> None:
        """Register VPC Agent memory hooks"""
        registry.add_callback(MessageAddedEvent, self.retrieve_vpc_context)
        registry.add_callback(AfterInvocationEvent, self.save_vpc_interaction)
        logger.info("VPC Agent memory hooks registered")
...
def create_agent_hooks(memory_id: str | None) -> list:
    ...
    session_id = str(uuid.uuid4())
    memory_hooks = VPCAgentMemoryHooks(
...
def create_devops_agent(model_id: str) -> Agent:
    """Create and configure the VPC agent."""
...
class ConversationManager:
    ...
    def __init__(self, agent: Agent, bot_name: str = "AWS-VPC-agent"):
        self.agent = agent
    ...
    def _list_all_mcp_tools(self) -> str:
        ...
        result += "• MCP configuration management tools (6 tools)\n\n"
        ...
        if aws_mcp_manager:
            try:
                ...
                result += "• Covers 50+ AWS services (VPC, EC2, S3, EKS, etc.)\n"
    ...
    def _show_help(self) -> str:
        ...
        help_text += "\n"
        help_text += "🌐 **VPC Expertise:**\n"
        help_text += "• Ask me anything about VPC, networking, security groups, routing!\n"
        
        return help_text
    
    def start_conversation(self):
        """Start the interactive conversation loop."""
        print(f"\n🚀 {self.bot_name}: Ask me about VPC and AWS! Type 'exit' to quit.")
        ...
        try:
            while True:
                try:
                    ...
                    if not user_input:
                        print(f"\n{self.bot_name} > Please ask me something about VPC on AWS!")
                        continue
    ...
    if gateway and gateway_id:
        try:
            ...
            gateway_access_token = get_token(
                get_ssm_parameter("/app/vpcagent/agentcore/machine_client_id"),
                get_cognito_client_secret(),
                get_ssm_parameter("/app/vpcagent/agentcore/cognito_auth_scope"),
                get_ssm_parameter("/app/vpcagent/agentcore/cognito_token_url")
            )
